...
public class Font_Old {
			...
			for (int unicode = 0x20; unicode <= 0x7E; unicode++) {
				...
				if (segmentIndex != -1) {
					if (idRangeOffset[segmentIndex] == 0) {
						...
						long offsetToGlyphArray = tableMetaData.offset + offset + 14 + (endCount.length * 2) + 2
								+ (startCount.length * 2) + (idDelta.length * 2) + (idRangeOffset.length * 2)
	...
	private final Glyph createFinalGlyph(boolean composite, int min_x, int max_x, int min_y, int max_y,
			boolean[] on_curve, int[] x, int[] y, int[] contour_ends) {
		...
		int[] finalContourEnds = new int[contour_ends.length];
		...
		for (int contour = 0; contour < contoursCount; contour++) {
			...
			boolean generatedStartPoint = false;
			int generatedStart = 0;
			...
			for (int point = contourStart; point <= contourEnd; point++) {
				if (point == contourStart) {
					if (!on_curve[point]) {
						generatedStart = finalGlyphPoints.size();
						finalGlyphPoints.add(new Vector2i((x[point] + x[contourEnd]) / 2, (y[point] + y[contourEnd]) / 2));
						finalGlyphFlags.add(true);
						
						generatedStartPoint = true;
				...
				if(point == contourEnd) {					
					...
					if(generatedStartPoint)
						startPoint = new Vector2i(finalGlyphPoints.get(generatedStart));
					else
						startPoint = new Vector2i(x[contourStart],y[contourStart]);
					
					if(!on_curve[point]) {
						if(!on_curve[point-1]) {
							finalGlyphPoints.add(new Vector2i((x[point] + x[point-1])/2,(y[point] + y[point-1])/2));
							finalGlyphFlags.add(true);
							contourImplicitPoints++;
						}	
						...
						finalGlyphPoints.add(startPoint);
						finalGlyphFlags.add(true);
						contourImplicitPoints++;
					}else {
						if(on_curve[point-1]) {
						...
						finalGlyphPoints.add(new Vector2i((x[point] + startPoint.x)/2, (y[point] + startPoint.y)/2));
						finalGlyphFlags.add(false);
						contourImplicitPoints++;
			...
			finalContourEnds[contour] = contour_ends[contour] + contourImplicitPoints;
		}

		int[] finalX = new int[finalGlyphPoints.size()];
		int[] finalY = new int[finalGlyphPoints.size()];
		boolean[] finalFlags = new boolean[finalGlyphPoints.size()];

		for (int i = 0; i < finalGlyphPoints.size(); i++) {
			finalX[i] = finalGlyphPoints.get(i).x;
			finalY[i] = finalGlyphPoints.get(i).y;
			finalFlags[i] = finalGlyphFlags.get(i);
		}

		return new Glyph(composite, min_x, max_x, min_y, max_y, finalFlags, finalX, finalY, finalContourEnds);
	}
	...
	private final void populateFontSSBOs() {
		...
		ByteBuffer pointsStructBuffer = BufferUtils
				.createByteBuffer(totalPointsCount * (2 * Float.BYTES + Integer.BYTES + 4/* padding */));
		ByteBuffer contoursStructBuffer = BufferUtils
				.createByteBuffer(totalContoursCount * Integer.BYTES * 8/* padding */);
		...
		for (Integer unicode : glyphs.keySet()) {
			...
			for (int contour = 0; contour < currentGlyph.getContoursCount(); contour++) {
				...
				contoursStructBuffer
						.putInt(currentGlyph.getContourEnd(contour) - currentGlyph.getContourStart(contour) + 1);
