...
uniform uint u_target_geometry;

void main() {
    Instance_s targetInstance = instances.instance_structs[u_target_geometry];
    Geometry_s target_geometry = geometries.geometry_structs[targetInstance.geometry_index];
    mat4 transform = transforms.transform_array[targetInstance.transform_index];
    ...
    if(gl_VertexID > target_geometry.point_count){
        ...
        gl_Position = vec4(vec2(pts.points_array[point_offset])/2644.5,0.0,1.0);
    }else{
        ...
        gl_Position = vec4(targetPoint.xy/2633.5, 0.0, 1.0);
    }
}